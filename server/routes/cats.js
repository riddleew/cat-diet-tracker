...
router.post('/', async (req, res, next) => {
  try {
    ...
    const [cat] = await sql`
      INSERT INTO cats (name, breed, notes)
      VALUES (${name}, ${breed || null}, ${notes || null})
      RETURNING *
    `;
...
router.put('/:id', async (req, res, next) => {
  try {
    ...
    const [cat] = await sql`
      UPDATE cats
      SET name=${name}, breed=${breed || null}, notes=${notes || null}
      WHERE id=${id}
      RETURNING *
    `;
    if (!cat) return res.status(404).json({ error: 'Cat not found' });
